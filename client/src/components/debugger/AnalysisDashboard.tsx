...
import { AnalysisResult } from '../../types/debugger';
...
export const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ result, onNewSession, onContinue, isAnalyzing, sessionHistory = [] }) => {
    const [followUp, setFollowUp] = useState('');

    // If sessionHistory is provided, use the latest result for the main display? 
    // Actually 'result' prop is usually the current/latest one.
    // We want to show older items above.

    // Filter out the current result from history to avoid duplication if it's passed in
    const displayHistory = sessionHistory.filter(h => h.analysisDate !== result.analysisDate || h.rootCauseAnalysis !== result.rootCauseAnalysis);
    ...
    return (
        <div className="w-full p-8 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-24 relative">
            ...
            {sessionHistory && sessionHistory.length > 1 && (
                <div className="mb-12 space-y-8 opacity-70 hover:opacity-100 transition-opacity">
                    <div className="flex items-center gap-4 text-neon-purple/50">
                        <div className="h-px bg-neon-purple/30 flex-1"></div>
                        <span className="text-xs font-bold uppercase tracking-widest">Previous Context</span>
                        <div className="h-px bg-neon-purple/30 flex-1"></div>
                    </div>

                    {sessionHistory.slice(0, -1).map((item, idx) => (
                        <div key={idx} className="glass-panel p-6 rounded-2xl border-white/5 bg-black/20">
                            <div className="flex justify-between items-start mb-4">
                                <div className="text-sm font-medium text-gray-400">Turn {idx + 1}</div>
                                <div className="text-xs font-bold text-neon-purple">{item.confidenceScore}/100</div>
                            </div>
                            <div className="mb-4">
                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Student Said</p>
                                <p className="text-sm text-gray-300 italic">"{item.originalExplanation}"</p>
                            </div>
                            <div>
                                <p className="text-xs text-neon-cyan/70 uppercase tracking-wider mb-1">AI Diagnosis</p>
                                <p className="text-sm text-white/80">{item.rootCauseAnalysis}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {/* Sticky Continuation Bar */}
            <div className="mt-8 glass-panel rounded-2xl p-4 sticky bottom-4 backdrop-blur-2xl border-white/20 shadow-[0_0_50px_rgba(0,0,0,0.5)] z-50">
                <div className="relative flex items-center gap-4">
                    <div className="flex-1">
                        <input
                            ...
                            placeholder="Ask a follow up question to deepen understanding..."
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') handleSend();
                            }}
                            className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-neon-purple/50 transition-all placeholder:text-gray-600 disabled:opacity-50"
                        />
                    </div>
                    <button
                        onClick={handleSend}
                        disabled={isAnalyzing || !followUp.trim()}
                        className="p-3 rounded-xl bg-neon-purple hover:bg-neon-purple/80 text-white transition-colors disabled:opacity-50"
                    >
                        {isAnalyzing ? <div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : <Send className="w-4 h-4" />}
                    </button>
                </div>
            </div>
